...
```mermaid
flowchart TB
    A[키패드 초기화] --> B[16바이트 AES 키 K 생성]
    B --> C[K를 RSA 공개키로 암호화<br/>→ 256바이트 저장]
    C --> D[사용자 'abc' 입력]
    D --> E[a 입력 → AES_K로 암호화<br/>→ 16바이트]
    E --> F[b 입력 → AES_K로 암호화<br/>→ 16바이트]
    F --> G[c 입력 → AES_K로 암호화<br/>→ 16바이트]
    G --> H[암호문 누적: 48바이트]
    H --> I{완료 버튼}
    I --> J[서버 전송 데이터 구성]
    J --> K[RSA암호화된 K 256바이트<br/>+ AES암호화된 abc 48바이트<br/>+ HMAC 20바이트<br/>= 총 324바이트]
    K --> L[서버 수신]
    L --> M[RSA 개인키로 K 복호화]
    M --> N[K로 abc 복호화]
```
